...
window.handlePremiumPayment = function(e) {
  ...
  try {
    ...
    if (isNaN(amount) || amount <= 0) { showToast('Lütfen geçerli bir tutar giriniz.', 'error'); return; }
      ...
      if (!isFullyPaid) {
        ...
        html += `<div class="fixed-ultra-card ${isOverdue ? 'overdue' : ''}" style="cursor:pointer;" onclick="openEditFixed('${f.id}')">
          ...
          <div style="display:flex; align-items:center; gap:10px; margin-top:12px; padding-top:12px; border-top:1px solid rgba(255,255,255,0.05);">
            <button class="btn-fixed-end" onclick="event.stopPropagation(); quickPayFixed('${f.id}', '${iterStr}', ${Math.round(remaining*100)/100})" style="background:var(--green); color:white; border:none; flex:1; height:44px; font-size:14px; cursor:pointer; -webkit-tap-highlight-color:transparent;">
